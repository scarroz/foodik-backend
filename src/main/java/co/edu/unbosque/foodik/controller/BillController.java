...
public class BillController {
    ...
    public ResponseEntity<ApiResponse<List<BillResponse>>> getMyBills(
        ...
        return ResponseEntity.ok(ApiResponse.ok(billService.getMyBills(userDetails.getUsername())));
    }

    @GetMapping("/reservation/{reservationId}")
    @Operation(summary = "Get bill by reservation ID")
    public ResponseEntity<ApiResponse<BillResponse>> findByReservation(
            @PathVariable UUID reservationId,
            @AuthenticationPrincipal UserDetails userDetails) {
        return ResponseEntity.ok(ApiResponse.ok(billService.findByReservation(reservationId, userDetails.getUsername())));
    }
