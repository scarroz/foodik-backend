...
public class BillServiceImpl implements BillService {
    ...
    public BillSummaryResponse split(UUID billId, SplitBillRequest request, String userEmail) {
        ...
        bill.setSplitMode(request.splitMode());
        BillSummaryResponse result = resolveStrategy(request.splitMode()).split(bill, request);
        try {
            com.fasterxml.jackson.databind.ObjectMapper mapper = new com.fasterxml.jackson.databind.ObjectMapper();
            bill.setSplitResult(mapper.writeValueAsString(result.amountPerUser()));
    ...
    @Override
    @Transactional(readOnly = true)
    public BillResponse findByReservation(UUID reservationId, String email) {
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BillResponse> getMyBills(String email) {
        User user = userRepository.findByEmail(email)
                .orElseThrow(() -> new ResourceNotFoundException("User not found"));
        return billRepository.findByReservationUserId(user.getId())
